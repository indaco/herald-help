// A real cobra CLI with a "greet" command and themed help via herald.
// Run:
//
//	cd examples/rich/002_cobra && go run . greet --name Alice
//	cd examples/rich/002_cobra && go run . --help
//	cd examples/rich/002_cobra && go run . greet --help
package main
...
func main() {
	...
	// Replace the default help function with herald-themed help.
	rootCmd.SetHelpFunc(func(cmd *cobra.Command, args []string) {
		hc := heraldcobra.FromCobra(cmd)
		hc.Footer = heraldhelp.FormatVersion("greeter", "1.0.0")
		_ = heraldhelp.RenderTo(cmd.OutOrStdout(), ty, hc, heraldhelp.WithStyle(heraldhelp.StyleRich))
	})
